"""Configuration manager for play vnt client.
...
class ConfigManager:
    ...
    def __init__(self, base_path: Optional[Path] = None):
        ...
        if base_path is None:
            base_path = Path.home() / ".playvnt"
            # Migration: Rename old .playpalace directory if it exists and new one doesn't
            old_path = Path.home() / ".playpalace"
            if not base_path.exists() and old_path.exists():
                try:
                    old_path.rename(base_path)
                    print(f"Migrated config directory from {old_path} to {base_path}")
                except Exception as e:
                    print(f"Error migrating config directory: {e}")
