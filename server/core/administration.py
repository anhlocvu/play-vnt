...
class AdministrationMixin:
    ...
    def _show_admin_menu(self, user: NetworkUser) -> None:
        ...
        items = [
            ...
            MenuItem(
                text=Localization.get(user.locale, "manage-accounts"),
                id="manage_accounts",
            ),
        ]
    ...
    def _show_manage_accounts_menu(self, user: NetworkUser) -> None:
        """Show menu to manage user accounts."""
        ...
        if not users:
            _speak_activity(user, "no-users-available")
            self._show_admin_menu(user)
            return

        items = []
        for username in users:
            items.append(MenuItem(text=username, id=f"manage_user_{username}"))
        items.append(MenuItem(text=Localization.get(user.locale, "back"), id="back"))
    ...
    def _show_user_management_actions_menu(self, user: NetworkUser, target_username: str) -> None:
        ...
        user.show_menu(
            "user_management_actions_menu",
            items,
            multiletter=True,
            escape_behavior=EscapeBehavior.SELECT_LAST,
        )
        self._user_states[user.username] = {
            "menu": "user_management_actions_menu",
            "target_username": target_username,
        }

    def _show_password_reset_editbox(self, user: NetworkUser, target_username: str) -> None:
        """Show editbox for entering new password."""
        prompt = Localization.get(user.locale, "password-reset-prompt", player=target_username)
        user.show_editbox(
            "reset_password",
            prompt,
            default_value="",
    ...
    def _show_delete_account_confirm_menu(self, user: NetworkUser, target_username: str) -> None:
        """Show confirmation menu for deleting an account."""
        _speak_activity(user, "confirm-delete-account", player=target_username)
        items = [
        ...
        self._user_states[user.username] = {
            "menu": "delete_account_confirm_menu",
            "target_username": target_username,
        }
    ...
    async def _handle_admin_menu_selection(
        ...
        elif selection_id == "manage_accounts":
            self._show_manage_accounts_menu(user)
    ...
    async def _handle_manage_accounts_selection(
        self, user: NetworkUser, selection_id: str
    ) -> None:
        """Handle manage accounts menu selection."""
        if selection_id == "back":
            self._show_admin_menu(user)
        elif selection_id.startswith("manage_user_"):
            target_username = selection_id[12:]  # Remove "manage_user_" prefix
            self._show_user_management_actions_menu(user, target_username)

    async def _handle_user_management_actions_selection(
        self, user: NetworkUser, selection_id: str, state: dict
    ) -> None:
        """Handle user management actions menu selection."""
        target_username = state.get("target_username")
        if not target_username:
            self._show_manage_accounts_menu(user)
            return
    ...
    async def _handle_password_reset_editbox(
        self, admin: NetworkUser, text: str, state: dict
    ) -> None:
        """Handle password reset editbox submission."""
        target_username = state.get("target_username")
        if not target_username:
    ...
    async def _handle_delete_account_confirm_selection(
        self, user: NetworkUser, selection_id: str, state: dict
    ) -> None:
        """Handle delete account confirmation menu selection."""
        target_username = state.get("target_username")
        if not target_username:
            self._show_manage_accounts_menu(user)
            return
        ...
        if selection_id == "yes":
            await self._delete_account(user, target_username)
        else:
            self._show_user_management_actions_menu(user, target_username)
    ...
    @require_admin
    async def _reset_password(self, admin: NetworkUser, username: str, new_password: str) -> None:
        """Reset a user's password."""
        # Use auth manager to reset password (it handles hashing)
        if hasattr(self, "_auth") and self._auth.reset_password(username, new_password):
            _speak_activity(admin, "password-reset-success", player=username)
            # Log the action
            self._notify_admins(
                "account-action", "accountactionnotify.ogg", exclude_username=admin.username
            )
        
        self._show_user_management_actions_menu(admin, username)

    @require_admin
    async def _delete_account(self, admin: NetworkUser, username: str) -> None:
        """Permanently delete a user account."""
        # Check if they are deleting themselves
        if username == admin.username:
            # Maybe don't allow this? Or allow it?
            # For now, let's allow it but warn or prevent if it's the last owner?
            # Actually, Administration.py tends to prevent demoting oneself as owner.
            pass
        ...
        if self._db.delete_user(username):
            _speak_activity(admin, "account-deleted", player=username)
