...
def test_dice_keeping_style_from_str_defaults_to_playvnt():
    assert DiceKeepingStyle.from_str("quentin_c") == DiceKeepingStyle.QUENTIN_C
    assert DiceKeepingStyle.from_str("invalid") == DiceKeepingStyle.PLAY_VNT
...
def test_user_preferences_to_dict_and_from_dict_round_trip():
    ...
    assert rebuilt.dice_keeping_style == DiceKeepingStyle.PLAY_VNT
