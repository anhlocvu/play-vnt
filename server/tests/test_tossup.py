...
class TestTossUpGameActions:
    ...
    def test_roll_play_vnt_bust(self):
        """Test bust condition in play vnt rules (all red)."""
        self.game.options.rules_variant = "play vnt"

        # Try multiple times to get a bust scenario
        # A bust in play vnt happens when green=0 and yellow=0 (all red)
        busted = False
        for attempt in range(50):
            # Reset player state for new attempt
        ...
        # We should have encountered at least one bust in 50 attempts
        assert busted, "Should encounter a bust scenario in play vnt rules"
...
class TestTossUpPlayTest:
    ...
    def test_play_vnt_rules_variant(self):
        """Test game with play vnt rules."""
        random.seed(789)

        game = TossUpGame(
            options=TossUpOptions(target_score=30, rules_variant="play vnt")
        )
        bot1 = Bot("Bot1")
        bot2 = Bot("Bot2")
        game.add_player("Bot1", bot1)
...
class TestTossUpPersistence:
    ...
    def test_full_state_preserved(self):
        ...
        game = TossUpGame(
            options=TossUpOptions(
                target_score=150, starting_dice=12, rules_variant="play vnt"
            )
        )
        user1 = MockUser("Alice")
        user2 = MockUser("Bob")
        ...
        assert loaded.options.rules_variant == "play vnt"
