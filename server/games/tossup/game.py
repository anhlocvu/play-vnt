"""
Toss Up Game Implementation for play vnt v11.
...
class TossUpOptions(GameOptions):
    ...
    rules_variant: str = option_field(
        MenuOption(
            default="Standard",
            value_key="variant",
            choices=lambda g, p: ["Standard", "play vnt"],
...
class TossUpGame(Game):
    ...
    Bust if all dice show red (play vnt) or if you have no greens and at least
    ...
    def _action_roll(self, player: Player, action_id: str) -> None:
        ...
        for _ in range(tossup_player.dice_count):
            if is_standard:
                ...
                # play vnt: Equal distribution (3-sided die)
        ...
        if is_standard:
            ...
            # play vnt: Bust if all red (no green, no yellow)
