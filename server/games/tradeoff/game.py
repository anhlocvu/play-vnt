"""
Tradeoff Game Implementation for play vnt v11.
...
class TradeoffGame(Game):
    ...
    def _handle_dice_key(self, player: Player, key_num: int) -> None:
        ...
        - play vnt style: Toggle die at index (key_num - 1) for keys 1-5
        ...
        if style == DiceKeepingStyle.PLAY_VNT:
    ...
    def _handle_dice_trade(self, player: Player, value: int) -> None:
        ...
        Only works in Quentin C style. Silent in play vnt style.
        """
        user = self.get_user(player)
        if not user:
            return
